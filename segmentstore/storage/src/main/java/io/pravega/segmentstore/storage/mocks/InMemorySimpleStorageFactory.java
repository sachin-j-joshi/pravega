...
public class InMemorySimpleStorageFactory implements SimpleStorageFactory {
    ...
    @Override
    public Storage createStorageAdapter(ChunkMetadataStore metadataStore) {
        synchronized (this) {
            if (null != singletonStorage) {
                return singletonStorage;
            }
            Storage storage = newStorage(executor, new InMemoryChunkStorage(executor), metadataStore);
            if (reuseStorage) {
    ...
    static Storage newStorage(Executor executor, ChunkStorage chunkStorage, ChunkMetadataStore metadataStore) {
