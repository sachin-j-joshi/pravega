...
public class ChunkedSegmentStorage implements Storage {
    ...
    private void handleException(String streamSegmentName, Throwable e) {
        if (null != e) {
            val ex = Exceptions.unwrap(e);
            if (ex instanceof StorageMetadataAlreadyExistsException) {
                throw new CompletionException(new StreamSegmentExistsException(streamSegmentName, ex));
            }
            if (ex instanceof StorageMetadataWritesFencedOutException) {
                throw new CompletionException(new StorageNotPrimaryException(streamSegmentName, ex));
            }
    ...
    private CompletableFuture<Void> collectGarbage(Collection<String> chunksTodelete) {
        ...
        for (val chunkTodelete : chunksTodelete) {
            futures[i++] = chunkStorage.openWrite(chunkTodelete)
                    .thenComposeAsync(handle -> chunkStorage.delete(handle), executor)
                    .thenRunAsync(() -> {
    ...
    public CompletableFuture<Boolean> exists(String streamSegmentName, Duration timeout) {
        ...
        return executeAsync(() -> {
            ...
            return tryWith(metadataStore.beginTransaction(streamSegmentName),
                    txn -> {
                        return txn.get(streamSegmentName).thenApplyAsync(storageMetadata -> {
                            SegmentMetadata segmentMetadata = (SegmentMetadata) storageMetadata;
    ...
    private class ChunkIterator {
        private MetadataTransaction txn;
        private SegmentMetadata segmentMetadata;
        private ArrayList<String> chunksList = new ArrayList<>();
        private String currentChunkName;
        private ChunkMetadata currentMetadata;

        public ChunkIterator(MetadataTransaction txn, SegmentMetadata segmentMetadata) {
            this.txn = txn;
            ...
            return Futures.loop(
                    ...
                    () -> txn.get(currentChunkName)
                            .thenApplyAsync(storageMetadata -> {
                                ...
                                chunksList.add(currentChunkName);
    ...
    private static class TruncateOperation implements Callable<CompletableFuture<Void>> {
        ...
        public TruncateOperation(ChunkedSegmentStorage chunkedSegmentStorage, SegmentHandle handle, long offset) {
            this.handle = handle;
            this.offset = offset;
                            ...
                            .thenComposeAsync(storageMetadata -> {
                                ...
                                return updateFirstChunk(txn)
                                        .thenComposeAsync(v -> {
                                            ...
                                            // Finally commit.
                                            return commit(txn)
                                                    .handleAsync((value, e) -> handleException(value, e), chunkedSegmentStorage.executor)
                                                    .thenComposeAsync(vv ->
    ...
    private static class ReadOperation implements Callable<CompletableFuture<Integer>> {
        ...
        public ReadOperation(ChunkedSegmentStorage chunkedSegmentStorage, SegmentHandle handle, long offset, byte[] buffer, int bufferOffset, int length) {
            this.handle = handle;
            this.offset = offset;
            this.buffer = buffer;
        ...
        private CompletableFuture<Void> findChunkForOffset(MetadataTransaction txn) {
            ...
            return Futures.loop(
                    ...
                    () -> txn.get(currentChunkName)
                            ...
                            .thenApplyAsync(v -> logEnd(timer1),
                                    chunkedSegmentStorage.executor)
                    , chunkedSegmentStorage.executor);
        }

        private Void logEnd(Timer timer1) {
    ...
    private static class WriteOperation implements Callable<CompletableFuture<Void>> {
        ...
        private List<ChunkNameOffsetPair> newReadIndexEntries = new ArrayList<ChunkNameOffsetPair>();
        ...
        public WriteOperation(ChunkedSegmentStorage chunkedSegmentStorage, SegmentHandle handle, long offset, InputStream data, int length) {
            this.handle = handle;
            this.offset = offset;
            this.data = data;
            this.length = length;
        ...
        private CompletableFuture<Void> collectGarbage() {
            if (!isCommited && chunksAddedCount > 0) {
                // Collect garbage.
                return chunkedSegmentStorage.collectGarbage(newReadIndexEntries.stream().map(entry -> entry.getChunkName()).collect(Collectors.toList()));
            }
            return CompletableFuture.completedFuture(null);
        }
        ...
        private CompletableFuture<Integer> writeToChunk(MetadataTransaction txn,
            ...
            return chunkedSegmentStorage.chunkStorage.write(chunkHandle, offsetToWriteAt, bytesCount, bis)
                    ...
                    .exceptionally(e -> {
                        ...
                        throw new CompletionException(ex);
                        //throw new CompletionException(new BadOffsetException(segmentMetadata.getName(), offset, offset));
                        /*try {
                            throw new BadOffsetException(segmentMetadata.getName(), chunkedSegmentStorage.chunkStorage.getInfo(chunkHandle.getChunkName()).get().getLength(), offset);
                        } catch (CompletionException cse) {
                            log.error("{} write - Error while retrieving ChunkInfo for {}.", chunkedSegmentStorage.logPrefix, chunkHandle.getChunkName());
    ...
    private static class DefragmentOperation implements Callable<CompletableFuture<Void>> {
        private MetadataTransaction txn;
        private SegmentMetadata segmentMetadata;
        private String startChunkName;
        private String lastChunkName;
        private ArrayList<String> chunksToDelete;
        private ChunkedSegmentStorage chunkedSegmentStorage;
        ...
        public DefragmentOperation(ChunkedSegmentStorage chunkedSegmentStorage, MetadataTransaction txn, SegmentMetadata segmentMetadata, String startChunkName, String lastChunkName, ArrayList<String> chunksToDelete) {
            this.txn = txn;
            this.segmentMetadata = segmentMetadata;
        ...
        private CompletableFuture<Integer> concatUsingAppend(ConcatArgument[] concatArgs) {
            ...
            i = 1;
            return Futures.loop(() -> i < concatArgs.length,
                    () -> {
                        readAtOffset = 0;
    ...
    private static class ConcatOperation implements Callable<CompletableFuture<Void>> {
        private long traceId;
        private Timer timer;
        private SegmentHandle targetHandle;
        private long offset;
        ...
        public ConcatOperation(ChunkedSegmentStorage chunkedSegmentStorage, SegmentHandle targetHandle, long offset, String sourceSegment) {
