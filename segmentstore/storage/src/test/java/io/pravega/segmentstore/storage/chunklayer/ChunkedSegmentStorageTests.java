...
public class ChunkedSegmentStorageTests extends ThreadPooledTestSuite {
    ...
    public void testSimpleScenarioWithNonAppendProvider() throws Exception {
        ...
        TestUtils.checkBlockIndexEntries(testContext.metadataStore, testSegmentName, testContext.config.getIndexBlockSize(), 0, 10);
        ...
        TestUtils.checkBlockIndexEntries(testContext.metadataStore, testSegmentName,  testContext.config.getIndexBlockSize(), 0, 14);
        TestUtils.checkChunksExistInStorage(testContext.chunkStorage, testContext.metadataStore, testSegmentName);
    ...
    public void testFailoverBehavior() throws Exception {
        ...
        TestUtils.checkBlockIndexEntries(testContext.metadataStore, testSegmentName, testContext.config.getIndexBlockSize(), 0, 30);
        TestUtils.checkChunksExistInStorage(testContext.chunkStorage, testContext.metadataStore, testSegmentName);
    }
    ...
    private void testOpenWriteAfterFailover(String testSegmentName, int ownerEpoch, int maxRollingLength, long[] chunks, int lastChunkLengthInStorage) throws Exception {
        ...
        TestUtils.checkBlockIndexEntries(testContext.metadataStore, testSegmentName, testContext.config.getIndexBlockSize(), 0, Arrays.stream(chunks).sum());
        TestUtils.checkChunksExistInStorage(testContext.chunkStorage, testContext.metadataStore, testSegmentName);
    }
    ...
    private void testSimpleConcat(TestContext testContext, int maxChunkLength, int nChunks1, int nChunks2) throws Exception {
        ...
        TestUtils.checkBlockIndexEntries(testContext.metadataStore, targetSegmentName, testContext.config.getIndexBlockSize(), 0, ((long) nChunks1 + (long) nChunks2) * maxChunkLength);
    ...
    private void testBaseConcat(TestContext testContext, int maxRollingLength, long[] targetLayout, long[] sourceLayout, long[] resultLayout) throws Exception {
        ...
        TestUtils.checkBlockIndexEntries(testContext.metadataStore, "target", testContext.config.getIndexBlockSize(), 0, Arrays.stream(resultLayout).sum());
    ...
    private void testConcatUsingAppendsAfterWriteFailure(long maxRollingSize,
        ...
        TestUtils.checkBlockIndexEntries(testContext.metadataStore, targetSegmentName, testContext.config.getIndexBlockSize(), 0, expectedLength);
        TestUtils.checkChunksExistInStorage(testContext.chunkStorage, testContext.metadataStore, targetSegmentName);
    }
    ...
    public void testTruncateNoOpTruncateOffset() throws Exception {
        ...
        val h = testContext.chunkedSegmentStorage.openWrite(testSegmentName).get();
        testContext.chunkedSegmentStorage.truncate(h, validStart, null).get();
        TestUtils.checkBlockIndexEntries(testContext.metadataStore, testSegmentName, 3, validStart, validEnd);
    ...
    public void testRepeatedTruncates() throws Exception {
        ...
        for (int truncateAt = 0; truncateAt < 9; truncateAt++) {
            ...
            TestUtils.checkBlockIndexEntries(testContext.metadataStore, testSegmentName, testContext.config.getIndexBlockSize(), truncateAt, 9);
            TestUtils.checkChunksExistInStorage(testContext.chunkStorage, testContext.metadataStore, testSegmentName);
    ...
    private void testTruncate(long maxChunkLength, long truncateAt, int chunksCountBefore, int chunksCountAfter, long expectedLength) throws Exception {
        ...
        TestUtils.checkBlockIndexEntries(testContext.metadataStore, testSegmentName, testContext.config.getIndexBlockSize(), truncateAt, expectedLength);
        TestUtils.checkChunksExistInStorage(testContext.chunkStorage, testContext.metadataStore, testSegmentName);
    }
    ...
    public void testReadWriteWithMultipleFailoversWithGarbage() throws Exception {
        ...
        for (int i = 1; i < 5; i++) {
            ...
            TestUtils.checkBlockIndexEntries(testContext.metadataStore, testSegmentName, testContext.config.getIndexBlockSize(), 0, Arrays.stream(lengthsArray).sum());
    ...
    public void testTruncateWithMultipleFailoversWithGarbage() throws Exception {
        ...
        for (int i = 1; i < 5; i++) {
            ...
            TestUtils.checkBlockIndexEntries(testContext.metadataStore, testSegmentName, testContext.config.getIndexBlockSize(), truncateAt, writeAt);
    ...
    public void testTruncateWithFailover() throws Exception {
        ...
        TestUtils.checkBlockIndexEntries(testContext.metadataStore, testSegmentName, testContext.config.getIndexBlockSize(), offset, offset + i);
        ...
        TestUtils.checkBlockIndexEntries(newTestContext.metadataStore, testSegmentName, newTestContext.config.getIndexBlockSize(), offset, offset);
    ...
    private void testReadWriteWithFixedSize(String testSegmentName, TestContext testContext) throws Exception {
        ...
        TestUtils.checkChunksExistInStorage(testContext.chunkStorage, testContext.metadataStore, testSegmentName);
        TestUtils.checkBlockIndexEntries(testContext.metadataStore, testSegmentName,
                testContext.chunkedSegmentStorage.getConfig().getIndexBlockSize(), 0, 28);
    ...
    private SegmentHandle populateSegment(TestContext testContext, String targetSegmentName, long maxChunkLength, int numberOfchunks) throws Exception {
        ...
        TestUtils.checkBlockIndexEntries(testContext.metadataStore, targetSegmentName, testContext.config.getIndexBlockSize(), 0, dataSize);
        TestUtils.checkChunksExistInStorage(testContext.chunkStorage, testContext.metadataStore, targetSegmentName);
        return h;
    }
