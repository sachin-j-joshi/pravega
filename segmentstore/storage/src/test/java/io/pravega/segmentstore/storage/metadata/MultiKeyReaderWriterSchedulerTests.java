        ...
        for (int i = 0; i < 10; i++) {
            ...
            futures[i] = readLock.lock()
                    .thenApplyAsync(v -> {
                        ...
                        array[c] = readLock;
                        seq[c] = n;
                        System.out.printf("Running %d%n", n);
                        return null;
        ...
        for (int i = 0; i < 10; i++) {
            ...
            futures[i] = writeLock.lock()
                    .thenApplyAsync(v -> {
                        ...
                        array[c] = writeLock;
                        seq[c] = n;
                        System.out.printf("Running writer %d%n", n);
                        return null;
                    }).whenCompleteAsync((v, ex) -> {
                        writeLock.unlock();
                    });
    ...
    public void testParallelReadsAfterWrite() {
        ...
        futures[0] = writeLock.lock()
                .thenApplyAsync(v -> {
                    ...
                    array[c] = writeLock;
                    seq[c] = 0;
                    System.out.printf("Running writer %d%n", 0);
                    return null;
                }).whenCompleteAsync((v, ex) -> {
                    writeLock.unlock();
                });
        for (int i = 1; i < 11; i++) {
            ...
            futures[i] = readLock.lock()
                    .thenApplyAsync(v -> {
                        ...
                        array[c] = readLock;
                        seq[c] = n;
                        System.out.printf("Running reader %d%n", n);
                        return null;
                    }).whenCompleteAsync((v, ex) -> {
                        readLock.unlock();
                    });
    ...
    public void testWriteAfterParallelReads() {
        ...
        futures[10] = writeLock.lock()
                .thenApplyAsync(v -> {
                    ...
                    array[c] = writeLock;
                    seq[c] = 10;
                    System.out.printf("Running writer %d%n", 10);
                    return null;
                }).whenCompleteAsync((v, ex) -> {
                    writeLock.unlock();
                });
        ...
        for (int j = 0; j < 100; j++) {
            for (int i = 0; i < 10; i++) {
                ...
                futures[n] = readLock.lock()
                        .thenApplyAsync(v -> {
                            ...
                            array[c] = readLock;
                            seq[c] = n;
                            System.out.printf("Running reader %d - %d%n", n, w);
                            return null;
                        }).whenCompleteAsync((v, ex) -> {
                            readLock.unlock();
                        });
            ...
            futures[n] = writeLock.lock()
                    .thenApplyAsync(v -> {
                        ...
                        array[c] = writeLock;
                        seq[c] = n;
                        System.out.printf("Running writer %d - %d%n", n, w);
                        return null;
                    }).whenCompleteAsync((v, ex) -> {
                        writeLock.unlock();
                    });
