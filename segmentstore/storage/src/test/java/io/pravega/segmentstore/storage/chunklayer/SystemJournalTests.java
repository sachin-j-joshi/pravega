...
public class SystemJournalTests extends ThreadPooledTestSuite {
    ...
    public void testInitialization() throws Exception {
        ...
        // Init
        SystemJournal journal = new SystemJournal(containerId, epoch, storageProvider, metadataStore, policy);

        Assert.assertEquals(epoch, journal.getEpoch());
        Assert.assertEquals(containerId, journal.getContainerId());
        Assert.assertEquals(policy.getMaxLength(), journal.getSegmentRollingPolicy().getMaxLength());
        Assert.assertEquals(epoch, journal.getEpoch());
    ...
    public void testInitializationInvalidArgs() throws Exception {
        ...
        AssertExtensions.assertThrows( "Should not allow null storageProvider",
                () -> new SystemJournal(containerId, epoch, null, metadataStore, policy),
                ex -> ex instanceof NullPointerException);

        AssertExtensions.assertThrows( "Should not allow null metadataStore",
                () -> new SystemJournal(containerId, epoch, storageProvider, null, policy),
                ex -> ex instanceof NullPointerException);
    ...
    public void testIsSystemSegment() throws Exception {
        ...
        long epoch = 1;
        val policy = new SegmentRollingPolicy(maxLength);
        val journal = new SystemJournal(containerId, epoch, storageProvider, metadataStore, policy);
    ...
    public void testSimpleBootstrapWithTwoFailovers() throws Exception {
        ...
        SystemJournal journalBefore = new SystemJournal(containerId, epoch, chunkStorageProvider, metadataStoreBeforeCrash, policy);
        journalBefore.setSystemSegmentsPrefix(systemSegmentName);
        journalBefore.setSystemSegments(getSystemSegments(systemSegmentName));
        long offset = 0;
        ...
        // Epoch 2
        epoch++;
        SystemJournal journalAfter = new SystemJournal(containerId, epoch, chunkStorageProvider, metadataStoreAfterCrash, policy);
        journalAfter.setSystemSegments(getSystemSegments(systemSegmentName));
        journalAfter.setSystemSegmentsPrefix(systemSegmentName);
    ...
    public void testSimpleBootstrapWithMultipleFailovers() throws Exception {
        ...
        for (int i = 1; i < 10; i++) {
            ...
            epoch++;
            ChunkMetadataStore metadataStoreAfterCrash = getMetadataStore();
            SystemJournal journalInLoop = new SystemJournal(containerId, epoch, chunkStorageProvider, metadataStoreAfterCrash, policy);
        ...
        SystemJournal journalFinal = new SystemJournal(containerId, epoch, chunkStorageProvider, metadataStoreFinal, policy);
